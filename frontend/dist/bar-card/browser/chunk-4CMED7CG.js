...
var EventManager = class _EventManager {
  ...
  constructor(plugins2, _zone) {
    ...
    const otherPlugins = plugins2.filter((p) => !(p instanceof DomEventsPlugin));
    this._plugins = otherPlugins.slice().reverse();
    const domEventPlugin = plugins2.find((p) => p instanceof DomEventsPlugin);
    if (domEventPlugin) {
...
function containsSegmentGroupHelper(container, containee, containeePaths, matrixParams) {
  if (container.segments.length > containeePaths.length) {
    const current2 = container.segments.slice(0, containeePaths.length);
    if (!equalPath(current2, containeePaths)) return false;
    if (containee.hasChildren()) return false;
    if (!matrixParamsMatch(current2, containeePaths, matrixParams)) return false;
    return true;
    ...
    const current2 = containeePaths.slice(0, container.segments.length);
    const next = containeePaths.slice(container.segments.length);
    if (!equalPath(container.segments, current2)) return false;
    if (!matrixParamsMatch(container.segments, current2, matrixParams)) return false;
...
function replaceSegment(current2, oldSegment, newSegment) {
  const children = {};
  Object.entries(current2.children).forEach(([outletName, c]) => {
    if (c === oldSegment) {
  ...
  return new UrlSegmentGroup(current2.segments, children);
}
...
var NavigationStateManager = class _NavigationStateManager extends StateManager {
  ...
  registerNonRouterCurrentEntryChangeListener(listener2) {
    ...
    this.nonRouterEntryChangeListener = this.nonRouterCurrentEntryChangeSubject.subscribe(({
      ...
      listener2(path, state, "popstate", !this.precommitHandlerSupported ? {
        replaceUrl: true
      } : {});
    });
...
function finalize2(rootScope, value) {
  if (isFrozen(value))
    return value;
...
var Immer2 = class {
  constructor(config) {
    ...
    if (isBoolean2(config?.autoFreeze))
      this.setAutoFreeze(config.autoFreeze);
    if (isBoolean2(config?.useStrictShallowCopy))
      this.setUseStrictShallowCopy(config.useStrictShallowCopy);
    if (isBoolean2(config?.useStrictIteration))
...
function createImmutableStateInvariantMiddleware(options = {}) {
  if (false) {
    ...
    var stringify3 = stringify22, getSerialize = getSerialize2;
    let {
...
var buildGetDefaultMiddleware = () => function getDefaultMiddleware(options) {
  ...
  if (true) {
    ...
    if (serializableCheck) {
      ...
      if (!isBoolean3(serializableCheck)) {
    ...
    if (actionCreatorCheck) {
      ...
      if (!isBoolean3(actionCreatorCheck)) {
      ...
      return (dispatch, getState, extra) => {
        ...
        if (signal2) {
          if (signal2.aborted) {
            ...
            signal2.addEventListener("abort", () => abort(externalAbortMessage), {
              once: true
            });
          }
        }
...
// src/app/core/store/auth.slice.ts
var initialState = {
  user: null,
  status: "idle",
  error: null
...
var { authStarted, authSucceeded, authFailed, signedOut } = authSlice.actions;
var authReducer = authSlice.reducer;

// src/app/core/store/cocktails.slice.ts
var initialState2 = {
  items: [],
  current: null,
  status: "idle",
...
var cocktailsSlice = createSlice({
  name: "cocktails",
  ...
  reducers: {
    ...
    mutationSucceeded: (state, action) => {
...
var { listStarted, listSucceeded, requestFailed, currentStarted, currentSucceeded, clearCurrent, mutationStarted, mutationSucceeded, deleteSucceeded, resetMutation } = cocktailsSlice.actions;
var cocktailsReducer = cocktailsSlice.reducer;
...
// src/app/core/store/redux-store.service.ts
var ReduxStoreService = class _ReduxStoreService {
  dispatch(action) {
  ...
  static {
    this.\u0275fac = function ReduxStoreService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _ReduxStoreService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ReduxStoreService, factory: _ReduxStoreService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ReduxStoreService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
// src/app/core/api/api-error.util.ts
...
//# sourceMappingURL=chunk-4CMED7CG.js.map
